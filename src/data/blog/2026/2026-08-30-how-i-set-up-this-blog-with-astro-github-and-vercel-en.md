...
description: "A simple guide on how to set a Astro Site"
pubDatetime: 2026-08-30T12:00:00+09:00
lang: en
tags: [blog, astro, github, vercel]
---

 # How I Made This Blog: A Beginner's Guide

> I'm using mac, Ghostty for terminal, and Zed for code editor. Sample project used in this blog post is here: [astro-blog-example](https://github.com/imjhk03/astro-blog-example)
...
## The four pieces
There are four pieces to understand before we do any thing to work.
### Astro
...
## AstroPaper
When using Astro, there are already made [Astro themes](https://astro.build/themes/1/) we can use to design our Astro blog. It already has many things that a blog needs:
- a home page
...
4. A text editor. VS Code is practical choice, but any editor will work.
5. (Optional) A domain name if you want an address such as ```myname.com```. Vercel provides a temporary deployment address while you are setting things up, like ```myname-vercel```
...
![](Screenshot%202026-08-30%20at%204.59.35%E2%80%AFPM.png)
...
![](Screenshot%202026-08-30%20at%204.58.50%E2%80%AFPM.png)
...
![](Screenshot%202026-08-30%20at%205.03.48%E2%80%AFPM.png)
...
![](Screenshot%202026-08-30%20at%205.05.39%E2%80%AFPM.png)
![](Screenshot%202026-08-30%20at%205.08.29%E2%80%AFPM.png)

The terminal will show a local address, usually `http://localhost:4321/`. Open that address in your browser.

![](Screenshot%202026-08-30%20at%205.06.27%E2%80%AFPM.png)
...
The main settings are in `src/config.ts`. A setting is a value that tells the website what to say or do. Open it to any code editor, for me I'm using Zed.
![](Screenshot%202026-08-30%20at%205.13.45%E2%80%AFPM.png)
...
export const SITE = {
  ...
  desc: "A short description of my blog",
  title: "Your Name",
  ogImage: "og.png",
...
- `website` is the final website address.
- `author` is your name.
- `profile` is the author's profile address.
- `desc` is a short explanation of the site.
- `title` is the site name.
- `ogImage` is the picture shown when a page is shared on social media.
- `lang` is the main language, such as `en` or `ko`.
- `timezone` tells the site how to show dates.

If `ogImage` is `"profile.jpg"`, the picture should be at `public/profile.jpg`.

This project has this line in `astro.config.mjs`:
```js
site: SITE.website,
```
...
If a different Astro project has a full address written directly in `astro.config.mjs`, update that address too.
...
│   └── utils/         # helper code
├── astro.config.mjs   # Astro's instructions
├── package.json       # list of tools
└── package-lock.json  # exact tool versions
```

The most important folder for writing is `src/data/blog/`. There won't be folder default in `src` folder, so create it.
![](Screenshot%202026-08-30%20at%205.34.18%E2%80%AFPM.png)
...
### (Optional) Write the same post in two languages
My blog has English and Korean posts. To connect two translations, make two files and add this information:

```yaml
lang: en
translations:
  en: /posts/2026/2026-08-30-my-first-post-en
  ko: /posts/2026/2026-08-30-my-first-post-ko
```

The Korean file uses `lang: ko` and points to the English file too. Do not put `.md` in the translation addresses.

In my project, the main post lists show English posts. Korean posts can still be opened directly or through the language switcher.
...
![](Screenshot%202026-08-30%20at%206.15.27%E2%80%AFPM.png)

When you want to check the finished production version, stop the development server and run:
```bash
npm run build
```

To stop the development server, input `opt + c`.

![](Screenshot%202026-08-30%20at%206.16.59%E2%80%AFPM.png)
...
![](Screenshot%202026-08-30%20at%206.17.38%E2%80%AFPM.png)
...
For a sample project that goes with this post, a clear repository name is `astro-blog-example`. A repository name is just the label on the project box. For my personal blog post, I used `jooheekim.me` which is the cusom domain I bought.
![](Screenshot%202026-08-30%20at%206.21.05%E2%80%AFPM.png)![](Screenshot%202026-08-30%20at%206.21.26%E2%80%AFPM.png)

Open terminal and in the pasth where you created the blog folder, run:
```bash
git init
git branch -M main
```
...
```
![](Screenshot%202026-08-30%20at%206.24.08%E2%80%AFPM.png)
...
```
![](Screenshot%202026-08-30%20at%206.27.35%E2%80%AFPM.png)
![](Screenshot%202026-08-30%20at%206.28.02%E2%80%AFPM.png)
...
![](Screenshot%202026-08-30%20at%209.38.25%E2%80%AFPM.png)
![](Screenshot%202026-08-30%20at%209.38.07%E2%80%AFPM.png)
- if you can't see your repository from the github, you may need to change the repository access from the github settings.

![](Screenshot%202026-08-30%20at%209.38.53%E2%80%AFPM.png)
![](Screenshot%202026-08-30%20at%209.39.43%E2%80%AFPM.png)
![](Screenshot%202026-08-30%20at%209.40.10%E2%80%AFPM.png)
...
For the sample project, it is https://astro-blog-example-three.vercel.app/.
...
8. Change `SITE.website` to the final HTTPS address.
9. Build and deploy again.

**DNS** is the address book that helps a domain find the correct server.
Do not guess DNS numbers from an old tutorial. Use the values shown by your current Vercel project. For more infomation, check vercel's guide [How do I add a custom domain to my Vercel project?](https://vercel.com/kb/guide/how-do-i-add-a-custom-domain-to-my-vercel-project)
...
npm run build
git add src/data/blog/2026/2026-08-30-my-second-post-en.md
...
1. Check that the computer is in the correct project folder.
...
4. Run `npm create astro@latest -- --template satnaing/astro-paper` to create a clean AstroPaper project.
5. Run `npm install`, then `npm run dev`. Make sure the starter opens locally.
...
7. Update `src/config.ts` with the person's title, name, description, domain, language, and timezone.
8. Check `astro.config.mjs`. Make sure its `site` address is the same as `SITE.website`.
...
13. Check `git remote -v` before connecting or pushing anything.
...
15. Only commit or push when the person has asked for that Git action.
16. Import the repository into Vercel with the Astro framework, `npm run build`, and `dist`.
17. Ask the person to complete any sign-in, permission, or DNS step that the agent cannot safely complete.
18. After deployment, check the Vercel address, the custom domain, one post, `/rss.xml`, `/sitemap-index.xml`, and `/search`.
...
After the first setup, I do not need to build a server or upload each page by hand. I write a file, check the build, save the change, and push it to GitHub.
